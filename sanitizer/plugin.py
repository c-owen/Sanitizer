...
# Translator for user-facing strings (see ``sanitizer/locale/*.json``). Untranslated
...
class SanitizerPlugin(BuzzPlugin):
    ...
    Responsibilities:
      ...
      * attach Sanitizer's UI (menu → review/restore) to the host on the main thread;
    ...
    def on_complete(self, transcription_id, task, segments, context) -> None:
        ...
        """
        try:
            from sanitizer_host.pipeline import sanitize_to_sidecar
        except Exception:  # noqa: BLE001 - core/host layer may be unavailable
            logger.exception("Sanitizer: pipeline unavailable; skipping sanitization.")
            return
        try:
            ...
            context.log.error(
                "Sanitizer: sanitization failed for %s: %s", transcription_id, exc
            )
    ...
    @staticmethod
    def _attach_ui() -> None:
        """Attach Sanitizer's menu to Buzz's main window, if a GUI is running.
        ...
        """
        try:
            from sanitizer_host.menu import attach_to_main_window
        except Exception:  # noqa: BLE001 - PyQt6/host UI may be unavailable
            logger.debug("Sanitizer: UI layer unavailable; running headless.")
            return
        try:
            attach_to_main_window()
        except Exception:  # noqa: BLE001 - never break plugin load on UI failure
            logger.exception("Sanitizer: failed to attach UI to the main window.")

...
Buzz: the host supplies an adapter (``sanitizer_host/model_provider_buzz.py``) and
...
from sanitizer_core.categories import ORG, PERSON, PLACE, PROJECT, label_for
...
# Model label (lower-cased) → Sanitizer category. Covers GLiNER zero-shot labels and
...
class ModelProvider(Protocol):
    ...
    in ``sanitizer_host`` (so the core never imports an ML lib); ``labels`` are a

...
from sanitizer_core import DeclaredListDetector, pii_detectors, restore, sanitize
...
class SanitizerDemoWindow(QWidget):
    """Sanitize → copy out → paste reply → restore, over ``sanitizer_core``."""

    def __init__(self, parent: QWidget | None = None) -> None:
        super().__init__(parent, Qt.WindowType.Window)
        self.setObjectName("sanitizer_demo_window")
        self.setWindowTitle(_("Sanitizer — Sanitizer demo"))
        self.resize(680, 760)
        self._last_scrubbed = ""
        self._key = Key()

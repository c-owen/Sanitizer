"""A :class:`~sanitizer_core.detectors.suggest.ModelProvider` backed by GLiNER, with
...
touches an ML library or Buzz, so ``sanitizer_core`` stays host-independent. The model
...
from sanitizer_core.detectors.suggest import RawEntity
...
# vendored in ``sanitizer/_vendor`` (no pip install); only these *weights* download, on
...
def _ensure_vendor_on_path() -> None:
    """Put Sanitizer's bundled third-party dir on ``sys.path`` so the vendored GLiNER
    imports with no pip install (see ``sanitizer/_vendor/``). No-op if already present
...
class BuzzGlinerProvider:
    ...
    def _ensure_downloaded(self) -> str:
        ...
        if base and base != self._repo_id:
            try:
                ...
                logger.exception(
                    "Sanitizer: base backbone %s could not be fetched", base
                )
        return gliner_dir
    ...
    def _ensure_repo(self, repo_id: str, patterns: list[str]) -> str:
        ...
        logger.info("Sanitizer: fetching %s via Buzz", repo_id)
        path = download_from_huggingface(
            repo_id, allow_patterns=patterns, progress=_NullProgress()
        )
        if not path:
            raise RuntimeError(
                f"Sanitizer: failed to download {repo_id!r} via Buzz's downloader"
            )
        return path

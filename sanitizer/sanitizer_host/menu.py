"""Main-thread UI attachment for Sanitizer.
...
SANITIZER_MENU_OBJECT_NAME = "sanitizer_menu"
REVIEW_ACTION_OBJECT_NAME = "sanitizer_review_action"
DEMO_ACTION_OBJECT_NAME = "sanitizer_demo_action"  # dev-only, gated by SANITIZER_DEV

_MAX_ATTACH_ATTEMPTS = 40
_RETRY_INTERVAL_MS = 50

# Strong references to Sanitizer's top-level windows, so neither Qt nor Python
...
def build_sanitizer_menu(main_window: QMainWindow) -> QMenu | None:
    ...
    existing = menu_bar.findChild(QMenu, SANITIZER_MENU_OBJECT_NAME)
    if existing is not None:
        return existing

    menu = menu_bar.addMenu(_("Sanitizer"))
    menu.setObjectName(SANITIZER_MENU_OBJECT_NAME)
    ...
    logger.info("Sanitizer: menu attached to '%s'.", main_window.windowTitle())
    return menu
...
def open_demo_window(parent: QWidget | None = None) -> QWidget:
    """Show the developer sanitizer playground (imported lazily; SANITIZER_DEV only)."""
    from sanitizer_host.demo_window import SanitizerDemoWindow

    return _show_window(SanitizerDemoWindow(parent))
...
def _safe_open(opener, parent: QWidget | None) -> None:
    ...
    try:
        ...
        logger.exception("Sanitizer: failed to open a window.")
...
def attach_to_main_window() -> None:
    """Schedule attaching the Sanitizer menu on the main thread's event loop.
    ...
    if QApplication.instance() is None:
        logger.debug("Sanitizer: no QApplication; skipping UI attach (headless).")
        return
...
def _attach_now(attempt: int) -> None:
    ...
    if main_window is not None:
        build_sanitizer_menu(main_window)
        return
    if attempt >= _MAX_ATTACH_ATTEMPTS:
        logger.warning(
            "Sanitizer: main window not found after %d attempts; UI not attached.",
            attempt,
        )
        return

# Sanitizer — a reversible, offline sanitizer for Buzz
...
1. **Transcribe** as usual. When Sanitizer is enabled, it sanitizes each finished
   transcript in the background and saves its work beside the transcript (your stored
   transcript is never modified).
2. **Sanitizer → Review & restore…** shows what was removed:
   ...
   - **Not touched — confirm these**: entity-shaped words Sanitizer left in, so a miss is
     as easy to catch as a find.
3. **Send out**: one button copies the scrubbed text. The key sits fenced below it.
4. **Restore**: paste the LLM's reply (markdown is fine) and Sanitizer fills the real

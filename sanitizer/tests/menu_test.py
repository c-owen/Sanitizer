...
def _reset_menu_state():
    ...
    from sanitizer_host import menu as menu_mod
...
def _sanitizer_menu(main_window):
    from PyQt6.QtWidgets import QMenu

    from sanitizer_host import menu as menu_mod

    return main_window.menuBar().findChild(QMenu, menu_mod.SANITIZER_MENU_OBJECT_NAME)


# --- build_sanitizer_menu (direct, synchronous) ---------------------------------


def test_build_sanitizer_menu_adds_menu(main_window):
    from PyQt6.QtGui import QAction

    from sanitizer_host import menu as menu_mod

    created = menu_mod.build_sanitizer_menu(main_window)

    assert created is not None
    assert _sanitizer_menu(main_window) is created
...
def test_build_sanitizer_menu_is_idempotent(main_window):
    from PyQt6.QtWidgets import QMenu

    from sanitizer_host import menu as menu_mod

    first = menu_mod.build_sanitizer_menu(main_window)
    second = menu_mod.build_sanitizer_menu(main_window)

    assert first is second
    menus = [
        m
        for m in main_window.menuBar().findChildren(QMenu)
        if m.objectName() == menu_mod.SANITIZER_MENU_OBJECT_NAME
    ]
    assert len(menus) == 1
...
def test_review_action_opens_window(main_window, qtbot):
    ...
    menu_mod.build_sanitizer_menu(main_window)
    ...
    qtbot.addWidget(window)
    assert window.objectName() == "sanitizer_review_window"
...
def test_demo_hidden_unless_dev_env(main_window, monkeypatch):
    ...
    monkeypatch.delenv("SANITIZER_DEV", raising=False)
    menu_mod.build_sanitizer_menu(main_window)
    assert main_window.findChild(QAction, menu_mod.DEMO_ACTION_OBJECT_NAME) is None
...
def test_demo_shown_with_dev_env(main_window, monkeypatch):
    ...
    monkeypatch.setenv("SANITIZER_DEV", "1")
    menu_mod.build_sanitizer_menu(main_window)
...
def test_attach_defers_then_attaches(main_window, qtbot):
    ...
    assert _sanitizer_menu(main_window) is None

    qtbot.waitUntil(lambda: _sanitizer_menu(main_window) is not None, timeout=2000)

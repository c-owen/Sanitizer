...
are behind an opt-in test (set ``SANITIZER_RUN_MODEL_TEST=1``) so CI never hits the
...
from sanitizer_core.model import TrustTier
...
def test_module_imports_without_ml_or_buzz():
    ...
    import sanitizer_host.model_provider_buzz as module
...
@pytest.mark.skipif(
    not os.environ.get("SANITIZER_RUN_MODEL_TEST"),
    reason="opt-in: set SANITIZER_RUN_MODEL_TEST=1 to download + run the real model",
)

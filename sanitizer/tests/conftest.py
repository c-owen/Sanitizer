"""Shared pytest setup for Sanitizer's host-integration tests.

Adds the plugin root and the Buzz package root to ``sys.path`` so that
``import sanitizer_core`` / ``sanitizer_host`` (and ``import buzz...``) resolve the same
...
_PLUGIN_ROOT = Path(__file__).resolve().parents[1]  # .../sanitizer

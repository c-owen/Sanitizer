...
from sanitizer_host.pipeline import build_detectors, sanitize_to_sidecar
...
class _Ctx:
    def __init__(self, config):
        self.config = config
        self.log = logging.getLogger("sanitizer.test")
...
def test_on_complete_never_raises_into_host(tmp_path, monkeypatch):
    ...
    import sanitizer_host.pipeline as pipeline
...
def test_build_detectors_unions_the_declared_store(tmp_path):
    add_declared_term(tmp_path, "person: Jane")  # in Sanitizer's store, not the config

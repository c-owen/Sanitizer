# Sanitizer — Developer Notes & Phase Handoff
...
> Full technical plan: [`../sanitizer-implementation-plan.md`](../sanitizer-implementation-plan.md).
> UX/GUI design hand-off brief: [`../sanitizer-ux-design-brief.md`](../sanitizer-ux-design-brief.md).
...
| **1 — Core domain + declared-list** | ✅ **done** (2026-06-30) | Pure `sanitizer_core`; declared tier + vault + sanitize/restore. |
...
| **Step E — run-once, triage-interactively** | ✅ **done — v0.7.1 release** (2026-07-01) | The ~222 answer: keep GLiNER's **confidence** on each item; a live **triage panel** (confidence slider + live count + type toggles + min-mentions + sort) filtering the computed set client-side (no re-run); **non-linear** review — SUGGESTIONS grouped by type with per-type + shown-wide **bulk** approve/reject, multi-select + Ctrl+Return/Ctrl+Backspace. `sanitizer_core` stays pure. Shipped after a 4-track repo audit (security/packaging/correctness/docs) — fixes below. |

**Current state at a glance:** `sanitizer_core` **v0.7.1** · **252 tests pass** on system
...
now backed by a build-failing test (`sanitizer/sanitizer_core/tests/guarantees_test.py`,
including the offline PG1), FR-14 extensibility is demonstrated, locale files are
completeness-checked, and there's a user-facing `sanitizer/README.md`. **Suggestions are now
...
**vendored** (`sanitizer/_vendor`), so no pip. **Open problem → next work:** on a real
...
`sanitizer-implementation-brief-for-claude-code.md`). One window, **three modes** (Review /
...
**pure** stores (`sanitizer_core/appstate.py` prefs, `sanitizer_core/declared_store.py` list; both
host-path-injected). **Unsafe withholds** the scrubbed text from every copy path in every
mode (PG7, tested). `sanitizer_core` stays pure. Next: **Phase 6** (lock PG1–PG8 in CI).

**What Phase 0 delivered & verified:**
- A loadable `BuzzPlugin` (`id="sanitizer"`, `version="0.0.0"`, no pip deps).
- The packaging tool produces `dist/sanitizer.zip` (`plugin.py` at the archive root).
...
**Sanitizer** menu (no duplicate), "Hello from Sanitizer…" opens its window, and the
menu persists as a single entry after restart.

**What Phase 1 delivered (`sanitizer_core`, pure Python — no buzz/Qt):**
...
- **Guarantee enforced in CI:** `boundary_test.py` AST-scans `sanitizer_core` and
  fails if it ever imports `buzz`/`PyQt6` (the "verifiable independently" mandate).
- Tests run on the **system Python** (no Qt): `python -m pytest sanitizer/sanitizer_core/tests`
  → `43 passed`; property tests (`hypothesis`) add 2 more where installed.
- **In-Buzz demo** (`sanitizer_host/demo_window.py`): the Sanitizer menu gained a
  **"Sanitizer (Phase 1 demo)…"** action — a playground that runs `sanitizer_core`
  inside the real app (paste terms + text → scrubbed + key + restore). It also
  proves `sanitizer_core` imports/runs inside Buzz ahead of Phase 5. CLI twin:
  `tools/demo_core.py`.
...
**What Phase 2 delivered (`sanitizer_core`, still pure):**
...
**What Phase 3 delivered (`sanitizer_core`, still pure):**
...
**What Phase 3.5 delivered (semantic placeholder labels — `sanitizer_core`, pure):**
...
- `sanitizer_core/detectors/suggest.py` (still pure): a `ModelProvider` **port**
...
- **Host adapter** `sanitizer_host/model_provider_buzz.py` (`BuzzGlinerProvider`): the
...
- **Tests:** `sanitizer_core/tests/suggest_test.py` (stub provider — tier/PENDING/not-in-key,
  ...
  **opt-in real-model run** gated on `SANITIZER_RUN_MODEL_TEST=1`). New public API exported
  from `sanitizer_core`: `ModelProvider`, `ModelSuggestionDetector`, `RawEntity`,
  `DEFAULT_LABELS`. `sanitizer_core` is **still buzz/Qt-free** (`boundary_test` green).
...
- **Transcript pipeline (`sanitizer_core/transcript.py`, pure).** `sanitize_transcript`
...
- **Sidecar persistence (`sanitizer_core/persistence.py`, pure, path injected).** Writes
...
- **Host pipeline (`sanitizer_host/pipeline.py`).** `on_complete` calls `sanitize_to_sidecar`:
  ...
  `sanitizer_host/paths.py::sidecar_dir(id)` (`<Buzz cache>/plugins_data/sanitizer/<id>`).
...
- **Review & restore window (`sanitizer_host/review_window.py`)** — replaces the demo for
  ...
  **decisions** table (placeholder/original/type/why/count/state, words not color —
  ...
  is dev-gated behind the `SANITIZER_DEV` env var (Step A), and the "About" action was dropped.
- **Tests (+38):** `transcript_test`, `persistence_test` (pure core); `pipeline_test`
  ...
  markdown, empty state). `sanitizer_core` still buzz/Qt-free (`boundary_test` green).
...
- The decisions table is now **editable**: a *Remove* checkbox per row. Tick → APPROVE,
  ...
  `sanitizer_core.apply_review(segments, items)` to **re-derive** the scrubbed text + key
...
- **Placeholder-on-approval:** approving a PENDING suggestion allocates a fresh,
  non-colliding token via `sanitizer_core.next_free_placeholder(existing, label, scheme)`;
...
```
buzz-plugin/
├── buzz/                     # the host app (NEVER modified by Sanitizer)
├── sanitizer/                    # the distributable plugin (zipped as sanitizer.zip)
│   ├── plugin.py             # BuzzPlugin entry point + sys.path bootstrap + UI attach
│   ├── sanitizer_core/           # HOST-INDEPENDENT core (no buzz/no Qt)
...
│   ├── sanitizer_host/           # Buzz/Qt adapters (may import PyQt6 + buzz)
│   │   ├── i18n.py           # shared translator -> plugin root's locale/
│   │   ├── menu.py           # main-thread menu attach (Review; demo gated by SANITIZER_DEV)
...
├── tools/
│   ├── package.py            # folder -> dist/sanitizer.zip
│   └── demo_core.py          # CLI demo of sanitizer_core sanitize/restore (no Buzz/Qt)
└── dist/
    └── sanitizer.zip             # build output
```
...
# 3) ingest in Buzz:  Help → Plugins → Add by URL → http://localhost:8000/sanitizer.zip
#    (Buzz downloads, validates, installs to ~/.cache/Buzz/plugins/sanitizer, auto-enables)
```

`PluginManager.add_from_url` uses `urlopen`, so `file://` also works:
`file:///<abs>/dist/sanitizer.zip`.

**Fast inner loop** (skip the zip while iterating): copy the folder straight in and
restart Buzz:
```bash
cp -r sanitizer/. ~/.cache/Buzz/plugins/sanitizer/        # Windows: %LOCALAPPDATA%\Buzz\plugins\sanitizer
```

**Updating an installed Sanitizer requires restarting Buzz** — see the gotcha in §6.
...
- **`sanitizer/sanitizer_core/tests`** — the pure core (no buzz/Qt). Runs on plain **system
  Python**: `python -m pytest sanitizer/sanitizer_core/tests`.
- **`sanitizer/tests`** — host-integration: `packaging_test`, `plugin_load_test` (light
  `buzz.plugins.loader`), `menu_test` + `demo_window_test` (need PyQt6 + pytest-qt).

| Environment | Command (from repo root) | Result | What skips |
|---|---|---|---|
| **System Python** (3.14 here) | `python -m pytest sanitizer` | ~252 pass | menu/demo (no PyQt6), property (no hypothesis), md-render (no markdown) |
| **`.venv-qt`** (3.12 + PyQt6 + hypothesis + markdown) | `QT_QPA_PLATFORM=offscreen .venv-qt/Scripts/python.exe -m pytest sanitizer` | ~311 pass | plugin_load (no platformdirs/buzz) |
...
```
In the **full Buzz env** (PyQt6 present): `cd buzz && uv run python -m pytest ../sanitizer`.
Conftests put the plugin root (+ `buzz` root) on `sys.path`, so cwd doesn't matter.
Lint everything: `uvx ruff check sanitizer tools` and `uvx ruff format sanitizer tools`.
...
- **Format & lint:** `ruff` + `ruff-format`, line length **88**, double quotes
  (config in `sanitizer/pyproject.toml`, matching the Buzz host).
  Run: `uvx ruff check sanitizer tools` and `uvx ruff format sanitizer tools`.
...
- **Docstrings:** PEP 257. Module + public class/function docstrings required. For
  `sanitizer_core` interfaces, state **Assumes:** and **Guarantees:** explicitly — the
...
- **Layering (hard rule, will be test-enforced from Phase 1):** `sanitizer_core` MUST NOT
  import `buzz` or `PyQt6`. All host coupling lives in `sanitizer_host`.
...
- **Import bootstrap.** Buzz execs `plugin.py` as a standalone module by path, so
  ...
  intra-plugin imports are therefore **flat top-level** (`import sanitizer_core`,
  `from sanitizer_host.menu import …`). The `# noqa: E402` on the `buzz` import in
  `plugin.py` is intentional — it must follow the `sys.path` insert.
- **Update caveat (restart required).** `sanitizer_core`/`sanitizer_host` get cached in
...
- **Headless rule is thread-affinity, not a UI ban.** Buzz's contract says hooks
  ...
  Sanitizer attaches UI from `plugin.__init__`, which runs **on** the main thread during
...
- **Menu attach must be marshaled to the main thread (the real bug we hit).** Buzz
  ...
  - **Fix (ours only, no Buzz change):** `attach_to_main_window` posts the work onto the
    **main thread's event loop** via a `_MainThreadInvoker` (queued signal; moved to the
    ...
    thread (fixes the duplicate); `build_sanitizer_menu` stays idempotent. Covered by
...
- **Locale path.** `sanitizer_host/i18n.py` points `plugin_gettext` at the **plugin root**
  (`plugin.py`'s dir), not `sanitizer_host/`, so it finds `sanitizer/locale/`.
...
- **Suggestion tier is held, not applied (Phase 4).** `ModelSuggestionDetector` emits
  ...
  exercised by the opt-in test (`SANITIZER_RUN_MODEL_TEST=1`).
- **GLiNER is vendored, not pip-installed (decision, 2026-07-01).** `gliner` is a small
  **pure-Python** wheel and **all its runtime deps are already in Buzz** (`torch`,
  ...
  Buzz's `uv.lock`). So we bundle the ~1 MB of GLiNER code in **`sanitizer/_vendor/gliner/`**
  (unpacked `gliner-0.2.27-py3-none-any.whl`, Apache-2.0) and put `sanitizer/_vendor` on
  ...
  `sanitizer/_vendor/gliner/` from the new wheel (see `sanitizer/_vendor/README.md`). `_vendor` is
...
- **KNOWN ISSUE — demo crashed Buzz on the *first* open, worked on the second.** Likely
  ...
  triggered by a one-time first-run hiccup (lazy `sanitizer_core` import, or first-render
  ...
  double-instantiate + first lazy `sanitizer_core` import). `_safe_open` now turns it into a
...
- **Never mutate the transcript (implemented in 5a — keep it that way).** Sanitizer uses the
  **`on_complete`** hook (returns `None`), NOT `after_transcription` (whose return value
  ...
  (`segments.json` + a separate `key.json`), under `<Buzz cache>/plugins_data/sanitizer/<id>`.
...
- **Sidecar is Sanitizer's working copy, derived state recomputable.** The scrubbed text and
...
flowchart TB
    subgraph DEV["Dev / CI (host-independent)"]
        SRC["sanitizer/ source"] -->|tools/package.py| ZIP["dist/sanitizer.zip"]
        ZIP -->|python -m http.server| URL["http://localhost:8000/sanitizer.zip"]
        SRC -. pytest .-> T["sanitizer/tests/*_test.py"]
    end
    ...
    subgraph SANITIZER["Sanitizer plugin"]
        PY["plugin.py\nSanitizerPlugin(BuzzPlugin)"]
        MENU["sanitizer_host/menu.py\nattach_to_main_window()"]
        PIPE["sanitizer_host/pipeline.py\nsanitize_to_sidecar"]
        REVIEW["sanitizer_host/review_window.py\nReview & restore"]
        CORE["sanitizer_core/ (pure)\ndetectors · sanitizer · transcript · persistence · verify"]
        SIDE["sidecar\n<cache>/plugins_data/sanitizer/<id>\nsegments · decisions · key · meta"]
        PY --> MENU --> REVIEW
        PY --> PIPE --> CORE
        PIPE -->|write| SIDE
        REVIEW -->|read| SIDE
...
test. Full spec: [`../sanitizer-implementation-plan.md`](../sanitizer-implementation-plan.md).

**Phase 6 delivered (done):**
- **`sanitizer/sanitizer_core/tests/guarantees_test.py`** — PG1–PG8 stated once, build-failing.
...
- **`sanitizer/sanitizer_core/tests/extensibility_test.py`** — FR-14: a new `MacAddressDetector`
  **and** a new `HtmlPreHandler` format handler, defined in the test via the public
  ...
  Nothing in `sanitizer_core` was edited — the modular mandate, proven.
- **`sanitizer/tests/locale_completeness_test.py`** — all 14 locale files: valid JSON object,
  no empty-string values (the AGENTS.md blank-UI trap).
- **`sanitizer/README.md`** — user-facing: the guarantees, the honest limits (NG2), and the
  "the key is the secret" model.

**Step D wiring worth remembering (where the new state lives):**
- Two **pure** stores under Sanitizer's data dir (host-path-injected, same pattern as
  `persistence.py`): `sanitizer_core/appstate.py` — `preferences.json`
  (`has_reviewed` / `auto_apply_suggestions` / `key_note_dismissed`); and
  `sanitizer_core/declared_store.py` — `declared_terms.json` (the growable in-window list).
- **Declared terms have two homes:** Buzz's plugin config (read-only to us) **∪** the
  ...
  dir (injected `base_dir` in tests, else `sanitizer_data_dir()`).
...
`_SuggestionWorker` (QThread) — `sanitizer_core.suggest_items` runs the detector over
...
+ the bulk actions). Also: exercise real GLiNER via opt-in `SANITIZER_RUN_MODEL_TEST=1`; and
...
- **Key-file permissions (sec L2).** `key.json` (the secret, PG8) is written with the
  ...
  `chmod 0600` on the key + `0700` on the sanitizer data dir on POSIX (no-op/ACL on Windows).

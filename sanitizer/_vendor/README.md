...
This directory holds third-party packages **bundled into the Sanitizer plugin zip** so
they import at runtime without a separate `pip install`. Sanitizer adds this directory to
`sys.path` before importing anything here (see
`sanitizer_host/model_provider_buzz.py::_ensure_vendor_on_path`).
...
- **Why vendored:** GLiNER powers Sanitizer's optional *suggestion* tier (undeclared

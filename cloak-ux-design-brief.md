# Cloak — UX/GUI Design Brief (hand-off to a design agent)
...
coherent UX direction* for Cloak's user-facing surfaces. You are encouraged to be
...
2. Then go deeper in the canonical sources (paths in **§11**): the product brief
   ...
   (`cloak/DEV_NOTES.md`), and the two existing windows
   (`cloak/cloak_host/review_window.py`, `cloak/cloak_host/demo_window.py`).
...
**Cloak** is an offline, reversible *sensitive-information sanitizer* that runs as a
plugin inside **Buzz** (a PyQt6 desktop transcription app). After a transcription
finishes, Cloak replaces sensitive items — the user's **declared terms** (people,
...
and Cloak **restores** the real values from a local **key**. The mental model we must
...
- **Fail-closed must be legible** (PG7/US9): when Cloak can't confirm the declared set
...
Everything Cloak flags belongs to exactly one tier, and the tier governs default
...
3. **`HelloWindow` — "About Cloak…"** — a Phase-0 skeleton remnant; effectively an about
   box.
...
- **PyQt6 desktop, plugin-only.** Cloak must **not modify Buzz**. There is **no
  UI-extension hook** in Buzz, so Cloak surfaces are **its own top-level `QWidget`
  windows**, launched from a "Cloak" menu Cloak attaches to Buzz's menu bar via public
  ...
  screen* is not.)
- **Modeless & coexisting.** Cloak windows float alongside Buzz's main window; the user
  may have Buzz and a Cloak window open together.
...
11. **Where do declared terms get managed?** In Buzz's settings dialog (current), inside
    the Cloak window, or both? Editing the list is core to the value (US2).
...
- **Product brief (binding):** `buzz-sanitizer-spec.md` — esp. **§4** (persona/stories),
  ...
  **§13** (risks).
- **Implementation plan:** `cloak-implementation-plan.md` — phasing, architecture, the
  §2.3 "no UI-extension hook" finding and the menu-injection resolution.
- **Developer notes (current state of the build):** `cloak/DEV_NOTES.md` — what's done,
  the data model, the threading/menu constraints (§6), and what's deferred (§8).
- **The actual windows to critique:**
  - `cloak/cloak_host/review_window.py` — the real review/restore surface.
  - `cloak/cloak_host/demo_window.py` — the developer playground.
  - `cloak/cloak_host/menu.py` — how Cloak's menu + windows attach to Buzz.
- **The data model the UI binds to:** `cloak/cloak_core/transcript.py` (`ReviewItem`,
  `Placement`, `SanitizedSegment`), `cloak/cloak_core/model.py` (`TrustTier`,
  `DecisionState`), `cloak/cloak_core/persistence.py` (the sidecar).

# Cloak UI — start-here handoff manifest

Everything needed to implement the Cloak review window UI, in one place. Read in this order.

## 1. Instructions (read first)
- **`../cloak-implementation-brief-for-claude-code.md`** — the build plan: region→widget→data
...
- **`cloak_layout_skeleton.py`** — the approved v2 design as a real PyQt6 widget tree,
  ...
  `python -c "import sys;sys.path.insert(0,'design');from cloak_layout_skeleton import CloakDesignSkeleton;from PyQt6.QtWidgets import QApplication as A;a=A([]);w=CloakDesignSkeleton();w.show();a.exec()"`
...
- **`handoff/Cloak-Test-Fit.html`** — the interactive design test-fit (open in a browser;
...
- **`../cloak-design-revision-v2.md`** — the design decisions and the ones deliberately
  declined (e.g. suggestions stay held-by-default, FR-9).
- **`../cloak-ux-critique-and-direction.md`** §4–6 — wireframes, the colour-independent
...
- `../cloak/cloak_host/review_window.py` (the surface to evolve), `../cloak/cloak_host/menu.py`
  (drop demo/About), `../cloak/cloak_core/` (the pure data model — **read, do not add Qt to it**).
...
never depends on colour alone — UX-5 · suggestions are never auto-applied — FR-9 · `cloak_core`

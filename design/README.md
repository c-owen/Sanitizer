# Sanitizer — design skeleton (the structural spec for the real UI)

This folder is the **buildable design handoff** for the Sanitizer review window. It is the
...
- **`sanitizer_layout_skeleton.py`** — layout-only window. Builds the approved v2 design as
...
```
python design/sanitizer_layout_skeleton.py   # add an app runner, or:
python -c "from PyQt6.QtWidgets import QApplication; import sys; \
  sys.path.insert(0,'design'); from sanitizer_layout_skeleton import SanitizerDesignSkeleton; \
  a=QApplication([]); w=SanitizerDesignSkeleton(); w.show(); a.exec()"
```
...
and the design docs (`../sanitizer-design-revision-v2.md`, `../sanitizer-ux-critique-and-direction.md`,
...
from `sanitizer/sanitizer_core`) into this layout — not to reinvent the layout. Follow the phased
plan in `../sanitizer-implementation-brief-for-claude-code.md`, starting with Step A.

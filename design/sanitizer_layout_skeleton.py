"""Sanitizer — layout-only PyQt6 skeleton (DESIGN SPEC, not production code).

This module is the *structural handoff* for the Sanitizer review window. It builds the
...
class SanitizerDesignSkeleton(QMainWindow):
    """Layout-only stand-in for the real ReviewWindow. No logic; render-driven."""

    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("Sanitizer — Review & restore")
        self.resize(940, 700)
    ...
    def _build_menubar(self) -> None:
        mb = self.menuBar()
        for name in ("File", "Edit", "View", "Sanitizer", "Help"):
            m = mb.addMenu(name)
            if name == "Sanitizer":
                for a in ("Review & restore…", "About / what Sanitizer protects…"):
    ...
    def _build_sendout(self) -> QWidget:
        ...
        self._sendout_wall = QLabel("⛔  BLOCKED — UNSAFE\n\nSanitizer could not confirm your declared "

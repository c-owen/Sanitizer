# Sanitizer — Phased Implementation Plan

**Plugin:** Sanitizer — reversible, offline sensitive-information sanitizer for Buzz
...
`PluginManager.add_from_url(url)` → `loader.download_and_extract(url)` → `urlopen(url, timeout=60)` → safe-extract (zip-slip guarded) → **validate by loading** → copy to `~/.cache/Buzz/plugins/<id>/` → auto-enable. Because it uses `urlopen`, a locally served `http://localhost:8000/sanitizer.zip` (or `file://…`) installs exactly like a remote one. Archive may have `plugin.py` at the root or inside one wrapping folder. **This is the brief's "serve the zipped plugin locally and ingest it into the app."**
...
- In `__init__`, Sanitizer locates the running `QMainWindow` via `QApplication.instance().topLevelWidgets()` and attaches its own **"Sanitizer" menu** through the **public `QMainWindow.menuBar()` API** (the same surface any Qt app exposes — not a Buzz internal). The menu actions open Sanitizer's own top-level `QWidget` windows (Review, Restore).
...
This is the single most novel integration assumption, so **Phase 0 de-risks it directly** with a trivial menu+window spike. Fallback if menu injection proves fragile across Buzz versions: Sanitizer shows a small always-available **floating launcher window** at startup instead (still public Qt only). The decision does not affect the host-independent core (Phases 1–4).

### 2.4 Don't mutate the transcript; persist a sidecar

`after_transcription`'s return value is **saved** to the DB. Sanitizer must **not** overwrite the real transcript with scrubbed text — the user's stored transcript stays pristine; the scrubbed copy is the thing they *copy out*. So Sanitizer treats segments **read-only** and writes a **sidecar** keyed by `transcription_id`:

```
~/.cache/Buzz/plugins_data/sanitizer/<transcription_id>/
...
### 2.7 Reference plugins to model Sanitizer on
...
```
sanitizer/                                  # the distributable plugin folder (zipped as sanitizer.zip)
  plugin.py                             # BuzzPlugin subclass — THIN host glue only (§Phase 5)
  sanitizer_core/                           # HOST-INDEPENDENT library (pure Python; no buzz/no Qt)
  ...
  sanitizer_host/                           # Buzz/Qt adapters (imported only inside the app)
...
- **`sanitizer_core` suite** — pure-Python pytest, **no Buzz, no Qt imports**. Runnable standalone (`uv run pytest sanitizer/sanitizer_core/tests`) so the core is "verifiable independently of Buzz" (DoD). Fast; this is where the guarantees live.
...
- **Property-based tests** (substring-safety FR-1, reversibility FR-7) via `hypothesis` — a **test-only** dependency of `sanitizer_core`, never shipped.

### 4.2 Dynamic testing (the zip → serve → ingest loop)

A `make`-style target / script (`tools/package.py`) produces `dist/sanitizer.zip` (contents = the `sanitizer/` folder). Canonical loop:

```bash
# 1. build the zip
uv run python tools/package.py            # -> dist/sanitizer.zip

# 2. serve it locally
cd dist && uv run python -m http.server 8000

# 3. ingest in Buzz:  Help → Plugins → Add by URL → http://localhost:8000/sanitizer.zip
#    (auto-extracts, validates, installs to ~/.cache/Buzz/plugins/sanitizer, auto-enables)
```

**Fast inner loop** (skip the zip while iterating): copy `sanitizer/` straight into `~/.cache/Buzz/plugins/sanitizer/` and restart Buzz. **Re-ingest after a change:** re-add by URL (the loader `rmtree`s and recopies the dest). Each phase below names the concrete in-app action to verify.
...
**Deliverables**
- `sanitizer/plugin.py` — minimal `BuzzPlugin` (`id="sanitizer"`, name/description, no hooks), loads cleanly.
- `sanitizer_host/menu.py` — main-thread attachment that adds a **"Sanitizer"** menu with one action **"Sanitizer: Hello"** opening a trivial top-level `QWidget`. Attached from `plugin.__init__` via `QApplication.instance().topLevelWidgets()` → `menuBar()`.
- `tools/package.py` — folder → `dist/sanitizer.zip`.
- `locale/` seeded with the 14 required JSON files (empty maps are fine to start).
- Dev docs: the §4.2 loop written into `sanitizer/README-dev.md`.

**Static tests**
- Plugin imports and exposes valid `metadata.id` (mirror [`plugin_system_test.py`](buzz/tests/plugins/plugin_system_test.py)).
- `load_plugin_from_dir(sanitizer)` returns one `BuzzPlugin`.
- `package.py` yields a zip whose root (or single wrapping dir) contains `plugin.py`, and `download_and_extract("file://…/sanitizer.zip")` installs it.
- `qtbot` test: given a dummy `QMainWindow`, menu attachment adds the "Sanitizer" menu and the action opens the window.

**Dynamic test**
- Serve `sanitizer.zip`, **Add by URL** in Buzz, confirm Sanitizer appears in **Help → Plugins**, is enabled, the **Sanitizer menu** is present, and "Hello" opens. Verifies §2.3 end-to-end in the real app.
...
**Dynamic test:** add a hidden/debug "Sanitize sample text" item to the Sanitizer window; paste declared terms + text, see consistent placeholders and a key. (Manual sanity in-app; correctness is owned by static tests.)
...
**Dynamic test:** **Copy scrubbed (markdown)** from the Sanitizer window → paste into a markdown renderer → copy rendered text back → **Restore** → originals returned; unmatched placeholders ignored.
...
- `ModelProvider` **port** (in `sanitizer_core`) so the core stays host-free; tests inject a stub.
- `sanitizer_host/model_provider_buzz.py`: adapter that fetches the model on **first use** via Buzz's downloader into the shared cache and reuses it offline (FR-13). Runs on **CPU**, cross-platform.
...
- `sanitizer_host/ui/review_window.py` (minimal): **Copy scrubbed text** (single prominent action), **paste → Restore**, key shown as the branded secret with copy-the-wrong-pane guard (UX-6, UX-7).
...
- **README** (user-facing): what Sanitizer guarantees, what it explicitly does **not** (NG2 — not compliance certification, no completeness on undeclared items), and the **"the key is the secret"** model.
...
- Final `dist/sanitizer.zip`; full §4.2 acceptance pass; (optional) bundle path documented (add `sanitizer` to `loader.BUNDLED_PLUGIN_IDS` + `Buzz.spec`).
...
|---|---|---|
| D1 | **UI entry point = a "Sanitizer" menu injected via public `QMainWindow.menuBar()` at main-thread init**, opening Sanitizer's own top-level windows; floating-launcher fallback. | Only way to get a user-opened screen without modifying Buzz (§2.3, §14). |

# Cloak — a reversible, offline sanitizer for Buzz

Cloak replaces sensitive information in a transcript — the names, PII, and codenames
...
values. Cloak keeps it in a separate file (`key.json`), never inside the scrubbed
text, and marks it clearly in the UI. Guard the key; share the scrubbed text freely.

## How you use it

1. **Transcribe** as usual. When Cloak is enabled, it sanitizes each finished
   transcript in the background and saves its work beside the transcript (your stored
   transcript is never modified).
2. **Cloak → Review & restore…** shows what was removed:
   - **Removed — guaranteed**: your declared terms + detected PII, already taken out.
   - **Suggestions**: a local model's guesses (if enabled) — *held for your click*,
     never removed automatically.
   - **Not touched — confirm these**: entity-shaped words Cloak left in, so a miss is
     as easy to catch as a find.
3. **Send out**: one button copies the scrubbed text. The key sits fenced below it.
4. **Restore**: paste the LLM's reply (markdown is fine) and Cloak fills the real
   values back from the key, flagging any placeholder it couldn't resolve.

If Cloak **can't confirm** your declared terms were removed, it shows **BLOCKED —
...
| **Fail-closed** | If removal of the declared set can't be confirmed, Cloak never presents the output as clean and never lets you copy it (PG7). |
...
(`cloak/cloak_core/tests/guarantees_test.py`).

## What Cloak does **not** do

- **Not a compliance certification (NG2).** Cloak reduces disclosure risk; it does not
...
- **No cloud anything (NG1).** No remote detection, inference, or telemetry — ever, by
  design, not just in this version.
...
candidates it did *not* remove — declare the ones that matter so future transcripts
...
# In Buzz: Help → Plugins → Add by URL → http://localhost:8000/cloak.zip → restart Buzz
```
...
Per transcript, under Buzz's cache (`…/Buzz/plugins_data/cloak/<transcription id>/`):
...
meta.json       version, the clean/unsafe flag, counts
```

Cloak's preferences and your added declared terms live alongside, in

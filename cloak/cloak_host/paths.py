"""Where Cloak keeps its sidecars — inside Buzz's cache, beside Buzz's own data.
...
``plugins_data/cloak`` for per-transcription sidecars.
"""
...
_DATA_SUBDIR = ("plugins_data", "cloak")


def cloak_data_dir() -> str:
    """Base directory for all Cloak sidecars (created lazily by writers)."""
...
def sidecar_dir(transcription_id) -> str:
    ...
    return os.path.join(cloak_data_dir(), str(transcription_id))

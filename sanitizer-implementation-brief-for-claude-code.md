# Sanitizer — Implementation Brief: test-fit → real PyQt6 plugin

**For:** the Claude Code agent working in the `buzz-plugin` repo.
**Goal:** turn the approved v2 design into the real plugin UI — natively in PyQt6, against the data model that already exists, without modifying Buzz and without touching `sanitizer_core`'s purity.

## How to use this brief

Read, in order: this file → `sanitizer-design-revision-v2.md` (the decisions) → `sanitizer-ux-critique-and-direction.md` §4–§6 (wireframes, visual language, interaction model) → the visual reference `Sanitizer Test-Fit.html` (open it; it's the picture of the target). Then read the code you'll change: `sanitizer/sanitizer_host/review_window.py`, `sanitizer/sanitizer_host/menu.py`, and the data model in `sanitizer/sanitizer_core/transcript.py` + `model.py`. Implement in the phases below, shipping and testing each before the next.

## The one rule that prevents wasted work

**Do not port the HTML.** `Sanitizer Test-Fit.html` is a bundled React/CSS-in-JS artifact; its DOM and styles have no Qt equivalent. It is a **visual spec only**. Rebuild the design as native Qt widgets by **evolving the existing `ReviewWindow`** (it already does sidecar loading, live re-derive, and persistence — keep that spine). The binding contract is the design docs + spec IDs, not the HTML's code.
...
3. **Empty-state scan evidence (US8).** "Scanned N detectors across M segments · 0 matches." The pipeline must record detector count + segment count into `meta` (small addition in `sanitizer_host/pipeline.py`); the empty state reads it back. A null result must *prove it ran*.
...
- **Keep `sanitizer_core` pure** — no `buzz`/`PyQt6` imports; `boundary_test.py` AST-scans and fails the build otherwise. All UI lives in `sanitizer_host`.
- **Never modify Buzz**; UI is Sanitizer's own top-level window, attached via the existing main-thread menu marshaling (`menu.py`). Main-thread UI only.
...
> Read `sanitizer-implementation-brief-for-claude-code.md`, then `sanitizer-design-revision-v2.md` and `sanitizer-ux-critique-and-direction.md` §4–6, and open `Sanitizer Test-Fit.html` as the visual target. Implement **Step A only** first: evolve `sanitizer/sanitizer_host/review_window.py` into the two-zone tree + collapsed cleartext + "Approve everything," make the unsafe state withhold the scrubbed text (not just disable copy), fence the key with a copy-confirmation toast, and drop the demo/About actions from `menu.py`. Keep `sanitizer_core` pure, don't modify Buzz, add tests (including a PG7 test that copy is truly impossible when unsafe), and keep the existing suite green. Show me the diff before moving to Step B.
